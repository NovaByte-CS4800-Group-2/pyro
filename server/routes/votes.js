...
router.post('/vote', async(req, res) =>{
  ...
  if(!content_id || !user_id || (!value && value != 0)) res.status(400).json({ error: "Missing value" });

  const voting = await Vote.vote(content_id, user_id, value);

  if(!voting) res.status(406).json({ errror: "Unable to vote"});
...
router.delete('/remove/votes', async(req, res) =>{  // NOTE: wantes an array of ids (used for removing from comment/post)
  ...
  if(!content_ids) res.status(400).json({ error: "Missing content IDS" });

  const deleted = await Vote.removeVotes(content_ids);

  if(!deleted) res.status(406).json({ errror: "Unable to remove votes"});
...
router.delete('/remove/vote/:content/:user', async(req, res) =>{ 
  ...
  if(!content || !user) res.status(400).json( { error: "Missing values" });

  const deleted = await Vote.removeVote(content, user);

  if(!deleted) res.status(406).json({ errror: "Unable to remove vote"});
...
router.get('/single/vote/:content/:user', async(req, res) => {  // rn returns the buffer, dont know how to return only 1 or 0
  ...
  if(vote === false) res.status(406).json({ error: "no vote exists"})
