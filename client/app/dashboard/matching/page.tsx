...
export default function Matching() {
	...
	useEffect(() => {
		const getApplication = async () => {
			const resApp = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get/user/form/${user?.uid}`, {
				method: "GET",
				headers: {
	...
	useEffect(() => {
		if (form.user_id != "") {
			const getMatches = async () => {
				...
				const resMatch = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/get/match/${form.form_id}/${form.type}`, {
					method: "GET",
					headers: {
						"Content-Type": "application/json",
					},
		...
		<div className="flex-grow flex">
			<div className="flex flex-col flex-grow items-center m-5">
				...
				<div className="flex flex-grow flex-wrap mt-10 gap-y-10">
					<Link className={"h-[120px] w-[230px] mx-10" + (form.user_id === "" ? "" : " pointer-events-none")} tabIndex={form.user_id === "" ? -1 : 0} aria-disabled={form.user_id === ""} href="/dashboard/matching/host">
					...
					<Link className={"h-[120px] w-[230px] mx-10" + (form.user_id === "" ? "" : " pointer-events-none")} tabIndex={form.user_id === "" ? -1 : 0} aria-disabled={form.user_id === ""}  href="/dashboard/matching/housing">
						<Card className="h-full w-full">
							<CardBody>
								<div className="flex justify-center items-center h-full">
				...
				{ form.user_id ?  
					( <div className="border-1 border-[--greige-mist] shadow-md py-2 px-3 rounded-lg mt-4 flex-col flex">
						...
						<div className="flex mt-2 gap-x-1">
							{matches.length > 0 && <Button color="primary" className="text-sm h-8" onPress={() => setShowMatches(true)}>Match!</Button>}
							<Button onPress={deleteForm} color="secondary" className="text-sm h-8 font-semibold">Delete</Button>
						</div>
